...
export function OkrObjectiveHeader({ title, weightage, editable, onChange, onRemove }: OkrObjectiveHeaderProps) {
  return (
    <Flex align="center" gap="8px">
      {editable ? (
        <Input size="sm" bg="white" px="12px" flex="1" placeholder="Objective title" value={title} onChange={(e) => onChange({ title: e.target.value })} />
      ) : (
        <Text fontSize="13px" fontWeight="700" color="grey.80" flex="1">{title || "Untitled objective"}</Text>
      )}

      {editable ? (
        <Input size="sm" bg="white" px="12px" fontSize="12px" type="number" w="90px" placeholder="Weightage %" value={weightage || ""} onChange={(e) => onChange({ weightage: Number(e.target.value) })} />
      ) : (
